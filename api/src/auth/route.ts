...
authRouter.post("/login", middlewareLogin, async function (req, res, next) {
    ...
    try {
        ...
        const signedToken = jsonwebtoken.sign({ email: userRecord[0].email, userId: userRecord[0].userId, role: userRecord[0].role, firstName: userRecord[0].firstName }, "PASSWORD123456789", { expiresIn: '60h' })
